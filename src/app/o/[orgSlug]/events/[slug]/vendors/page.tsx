...
export default async function VendorRegistrationPage({ params }: { params: { orgSlug: string; slug: string } }) {
    ...
    <main>
      <header className="sticky top-0 z-10 bg-white/80 backdrop-blur ring-1 ring-slate-200">
        <div className="mx-auto flex max-w-3xl items-center justify-between px-4 py-3">
          <Link href={`/o/${params.orgSlug}/events/${event.slug}`} className="text-sm text-brand-700">◀ Back to event</Link>

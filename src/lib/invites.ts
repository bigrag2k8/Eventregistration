...
export async function sendInviteEmail(p: SendInviteParams) {
  ...
  // Team-focused phrasing for staff/volunteer/organizer/admin invites: "X invited
  // you to join the {Org}'s team". The role still appears below the intro so the
  // invitee knows what they're signing up for. The setup-org branch (no role) is
  // a different flow and keeps its original wording.
  const subject = roleLabel
    ? `${p.inviterName ?? "You"} invited you to join the ${p.orgName}’s team`
    : `You're invited to set up ${p.orgName} on Your Events App`;

  const intro = roleLabel
    ? `${p.inviterName ? `${esc(p.inviterName)} has` : "You've been"} invited you to join the <strong>${esc(p.orgName)}</strong>’s team.`
  ...
  <div style="font-family:Arial,sans-serif;max-width:600px;margin:24px auto;padding:24px;background:#fff;border-radius:12px">
    ...
    <p>${intro}</p>
    ${roleLabel ? `<p style="color:#475569;margin:4px 0 0;font-size:14px">Your role: <strong>${esc(roleLabel)}</strong></p>` : ""}
